...
if [[ $terraform_previous_env != ${terraform_config}_$terraform_env || ! -f ./.terraform/terraform.tfstate ]]; then
  mv -f .terraform/terraform.tfstate .terraform/terraform.tfstate.$terraform_previous_env > /staging/null 2>&1
  mv -f .terraform/terraform.tfstate.backup .terraform/terraform.tfstate.backup.$terraform_previous_env > /staging/null 2>&1
  terraform remote config \
    -backend=s3 \
    -backend-config=bucket=weareweekday.com-terraform-state-files \
    -backend-config=region=eu-west-1 \
    -backend-config=key=$terraform_config/$terraform_env \
    -backup=-
